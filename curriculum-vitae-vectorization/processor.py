    ...
    for row in candidate_records:
        ...
        try:
            ...
            resume = {
                ...
                'content': sanitize_content(resume_content),
                'embedding': embedding
            }
            repository.insert_candidate_vector(resume)
            info(f'Finished process for candidate {candidate_id}...\n')
        except Exception as e:
            error(f"An error occurred while processing data from candidate: {candidate_id}: {e}")
    repository.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Type of integration: 1 = ALL_CANDIDATES, 2 = ONLY_NOT_PROCESSED_YET (default: 1)"
    )
    parser.add_argument(
        "-k", "--aws_key",
        type=str,
